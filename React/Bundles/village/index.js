...
class BundlesPage extends Component {
    ...
    static async getInitialProps(options) {
        ...
        // Редирект на /bundles если в городе нет тарифов для частного дома
        const villageTariffs = getTariffsSelector(store.getState()).filter(item => item.salePackage2in1.is_cottage > 0);
        const hasVillageTariffs = Boolean(villageTariffs && villageTariffs.length);
        if (!hasVillageTariffs) {
            redirectingInGetInitialProps(res, '/bundles');
        }
        ...
        await import('~/src/components/Bundles/Content/content_v.json').then(resp => {
    ...
    render() {
        ...
        return (
            <Layout
                ...
                relCanonical={link}
            >
                <Bundles tariffs={tariffs} title={titleH1} pageContent={pageContent} isVillage={true} compact={compact}/>
            </Layout>
        );
    }
}
